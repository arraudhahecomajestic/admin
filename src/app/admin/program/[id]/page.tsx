...
export default async function EditProgramPage({ params }: { params: { id: string } }) {
  ...
  const bilBayar = pend.filter((x) => x.status_bayar === "dibayar").length;
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-xl bg-white p-5 shadow-sm">
        <form action={kemasProgram} className="grid gap-3 sm:grid-cols-2">
          ...
          <label className="flex items-center gap-2 text-sm text-slate-600 sm:col-span-2"><input type="checkbox" name="berbayar" defaultChecked={p.berbayar} /> Program berbayar (borang pendaftaran peserta + bayaran CHIP)</label>
        ...
        <section className="rounded-xl bg-white shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-2 border-b px-5 py-3">
            ...
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="rounded-lg bg-green-100 px-3 py-1 font-semibold text-green-700">{bilBayar} dibayar{p.had_peserta ? ` / ${p.had_peserta}` : ""}</span>
            </div>
          </div>
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-slate-50 text-xs uppercase text-slate-500">
                <tr>
                  ...
                  <th className="px-4 py-2">Kesihatan / Kecemasan</th>
                  <th className="px-4 py-2">Foto</th>
                  <th className="px-4 py-2">Bayaran</th>
                  <th className="px-4 py-2"></th>
                </tr>
              </thead>
              <tbody>
                {pend.length === 0 && <tr><td colSpan={7} className="px-4 py-8 text-center text-slate-400">Belum ada pendaftaran peserta.</td></tr>}
                {pend.map((r, i) => {
                  ...
                  return (
                    <tr key={r.id} className="border-b align-top last:border-0">
                      ...
                      <td className="px-4 py-2.5">
                        <div className="font-medium text-slate-800">{r.nama_peserta}</div>
                        <div className="text-xs text-slate-500">{[r.umur ? `${r.umur} thn` : null, r.jantina, r.sekolah].filter(Boolean).join(" · ") || "—"}</div>
                      </td>
                      <td className="px-4 py-2.5">
                      ...
                      <td className="px-4 py-2.5">
                        {r.status_bayar === "dibayar"
                          ? <span className="rounded bg-green-100 px-2 py-0.5 text-xs font-semibold text-green-700">Dibayar</span>
                          : r.status_bayar === "percuma"
                          ? <span className="rounded bg-slate-100 px-2 py-0.5 text-xs font-semibold text-slate-600">Percuma</span>
                          : <span className="rounded bg-amber-100 px-2 py-0.5 text-xs font-semibold text-amber-700">Menunggu</span>}
                      </td>
                      <td className="px-4 py-2.5 text-right">
                        {w && <a href={`https://wa.me/${w}`} target="_blank" rel="noopener noreferrer" className="rounded-lg bg-green-600 px-3 py-1 text-xs font-semibold text-white hover:bg-green-700">WA</a>}
                      </td>
                    </tr>
                  );
                })}
