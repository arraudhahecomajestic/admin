...
export function ms(d: string | null) {
  if (!d) return "";
...
// Gaya kongsi untuk borang — dipakai di halaman admin/cetak & portal ahli.
export function GayaBorang() {
  return (
    <style>{`
      @media print {
...
export default function Borang({ a, nama = NAMA_SURAU }: { a: any; nama?: string }) {
  const tempoh = a.tempoh_menetap_nilai ? `${a.tempoh_menetap_nilai} ${a.tempoh_menetap_unit || ""}` : "";
  return (
    <article className="borang relative mx-auto max-w-[800px] rounded-lg bg-white p-8 shadow-sm" style={{ fontFamily: "serif" }}>
      {/* No. Rujukan ahli — sudut atas kanan */}
      <div className="absolute right-6 top-5 text-right" style={{ fontFamily: "system-ui, Arial, sans-serif" }}>
        <div className="text-[10px] uppercase tracking-wide text-slate-500">No. Rujukan</div>
        <div className="text-sm font-bold tracking-wide text-slate-900">{a.no_ahli || "-"}</div>
      </div>
      ...
      <header className="mb-4 flex items-center justify-between gap-3">
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <p className="text-xs text-slate-600">No. Pendaftaran Surau (JAIS): <b>{NO_PENDAFTARAN}</b></p>
        </div>
        {/* eslint-disable-next-line @next/next/no-img-element */}
