...
export default async function KhairatSelesaiPage({ searchParams }: { searchParams: { ref?: string; gagal?: string } }) {
  ...
  const mesej =
    status === "dibayar"
      ? `Terima kasih. Yuran khairat${jumlah ? ` sebanyak ${rm(jumlah)}` : ""} telah diterima. Keahlian khairat anda kini AKTIF untuk tahun ini.`
      : status === "gagal"
      ? "Pembayaran tidak berjaya atau dibatalkan. Anda boleh cuba semula dari Portal Saya."
  ...
  return (
    <div className="mx-auto max-w-lg rounded-xl bg-white p-8 text-center shadow-sm">
      ...
      <div className="mt-6">
        <Link href="/ahli" className="rounded-lg bg-surau px-5 py-2.5 text-sm font-semibold text-white hover:bg-surau-dark">Kembali ke Portal Saya</Link>
      </div>
    </div>
  );
}
