...
import MaklumBalasProgramForm from "@/components/MaklumBalasProgramForm";

export const dynamic = "force-dynamic";

export default async function MaklumBalasProgramPage({ params }: { params: { id: string } }) {
  if (!adminConfigured) return <p className="text-center text-slate-500">Sistem belum dikonfigurasi.</p>;

  const db = createAdminClient();
  const { data } = await db
    .from("program")
    .select("id, tajuk, tarikh, masa, lokasi, maklumbalas_dibuka")
    .eq("id", params.id)
    .maybeSingle();
  const p: any = data;
    ...
    <div className="mx-auto max-w-xl space-y-5">
      <div className="overflow-hidden rounded-xl bg-white shadow-sm">
        <div className="bg-surau/10 px-6 py-4 text-center">
          <p className="text-xs font-semibold uppercase tracking-wide text-surau">Maklum Balas Program · Surau Ar-Raudhah</p>
        </div>
        <div className="p-6">
          ...
          <p className="mt-2 text-sm text-slate-500">Kongsikan pandangan anda tentang program ini. Ia membantu kami menambah baik program akan datang.</p>
        </div>
      </div>

      {p.maklumbalas_dibuka ? (
        <MaklumBalasProgramForm programId={p.id} />
      ) : (
        <div className="rounded-xl border-2 border-amber-300 bg-amber-50 p-5 text-center text-sm font-semibold text-amber-800">
          Borang maklum balas untuk program ini belum dibuka atau telah ditutup. Terima kasih.
        </div>
      )}
