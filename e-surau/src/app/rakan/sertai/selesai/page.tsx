...
export default async function PenajaSelesaiPage({ searchParams }: { searchParams: { ref?: string; gagal?: string } }) {
  ...
  if (adminConfigured && ref) {
    ...
    const { data } = await db
      .from("bayaran").select("*").eq("no_rujukan", ref)
    ...
    if (b) {
      ...
      if (b.status === "menunggu" && b.chip_id) {
        try { if ((await laksanakanBayaran(b.chip_id)).dibayar) status = "dibayar"; } catch { /* biar */ }
      }
    }
  }
  ...
  const tajuk = status === "dibayar" ? "Tajaan Diterima" : status === "gagal" ? "Pembayaran Tidak Berjaya" : "Pembayaran Sedang Diproses";
  const mesej =
    status === "dibayar"
      ? `Jazakumullah khairan. Tajaan${jumlah ? ` sebanyak ${rm(jumlah)}` : ""} telah diterima. Logo & penyenaraian anda akan dipaparkan di portal sepanjang tempoh tajaan.`
      : status === "gagal"
      ? "Pembayaran tidak berjaya atau dibatalkan. Anda boleh cuba semula."
      : "Pembayaran anda sedang disahkan. Logo akan dipaparkan sebaik bayaran disahkan.";
